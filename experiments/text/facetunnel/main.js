...
//! TUNNELCODE from : https://tympanus.net/codrops/2017/05/09/infinite-tubes-with-three-js/ 
...
Tunnel.prototype.updateCameraPosition = function() {
  if (!Facepositions) { return; }
  var nosePos = { x: map_range(Facepositions[62][0], 200, 300, 0, 1), y: map_range(Facepositions[62][0], 200, 300, 0, 1) };

  // Rotate Z & Y axis
  this.camera.rotation.z = nosePos.x * 0.2;
  this.camera.rotation.y = Math.PI - nosePos.x * 0.06;
...
Tunnel.prototype.updateCurve = function() {
  ...
  if (!Facepositions) { return; }
  var nosePos = { x: map_range(Facepositions[62][0], 200, 300, 0, 1), y: map_range(Facepositions[62][0], 200, 300, 0, 1) }; // map_range


  // Update the points along the curve base on mouse position
  this.curve.points[2].x = -nosePos.x * 0.1;
  this.curve.points[4].x = -nosePos.x * 0.1;
  this.curve.points[2].y = nosePos.y * 0.1;
