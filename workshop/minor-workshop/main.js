...
function loadModelThingies() {
	...
	loader.load('./models/twist.glb', function (gltf) {
...
function createGrid(x, y, model) {
	...
	for (var i = 0; i < x; i++) {
		for (var j = 0; j < y; j++) {
			...
			newModel.morphTargetInfluences[2] = north;
			newModel.morphTargetInfluences[3] = east;


			newModel.morphTargetInfluences[4] = THREE.Math.mapLinear(
				theData[count].U,
				0,
				100, 
				0,
